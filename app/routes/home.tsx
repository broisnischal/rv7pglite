  ...
  return (
    <main className="pt-16 p-4 container mx-auto">
      ...
      <div className="flex my-5 align-top gap-5">
        <div className="w-1/3">
          <fetcher.Form ref={formRef} method="post" className="flex flex-col  gap-2 [&>input]:p-2 [&>input]:border w-full"  >
            <input type="text" name="firstname" placeholder="First Name" />
            <input type="text" name="lastname" placeholder="Last Name" />
            <input type="text" name="email" placeholder="Email address" />
            <input type="text" name="phone" placeholder="Phone number" />
            <button type="submit" disabled={busy} onClick={() => {
        ...
        <div className="flex gap-4 flex-wrap">
          {loaderData.contacts.length <= 0 ? (<div className="py-10">
            <h1>No Contacts in your phonebook!</h1>
          </div>) : loaderData.contacts.map((item) => (
